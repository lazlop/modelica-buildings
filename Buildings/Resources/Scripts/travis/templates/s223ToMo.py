...
mbl_path = os.path.join('..', '..', '..', '..', '..', 'Buildings')
# Parse conf.yml.
...
def simulate_case(arg, simulator, experiment_attributes):
    ...
    output_dir_path = tempfile.mkdtemp(
        prefix=output_dir_prefix, dir='/home/akprakash/Programming/spawn/modelica-buildings'
    )
    ...
    if simulator == 'dymola':
        ...
        s.addPreProcessingStatement(
            r'openModel("../Buildings/package.mo");'
        )
        s.addPreProcessingStatement('cd(dir="../{}")'.format(output_dir_path.split(os.sep)[-1]))
